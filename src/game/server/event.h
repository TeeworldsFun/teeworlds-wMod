...
enum {NOTHING, HAMMER, GUN, SHOTGUN, GRENADE, RIFLE, KATANA, UNLIMITED_AMMO, LIFE_ARMOR_CRAZY, SURVIVOR, PROTECT_X2, INSTAGIB, WALLSHOT, BULLET_PIERCING, BULLET_BOUNCE, BULLET_GLUE, HAVE_ALL_WEAPON, GRAVITY_0_25, GRAVITY_0, GRAVITY_M0_25, GRAVITY_M0_5, BOUNCE_10, HOOK_VERY_LONG, SPEED_X10,  WEAPON_SLOW, RACE_HUMAN, RACE_GNOME, RACE_ORC, RACE_ELF, RACE_RANDOM, JUMP_UNLIMITED, JUMP_X1_5, CURVATURE_0, ALL, END};
...
public:
	...
	{
		if (Event == m_ActualEvent[0] || (g_Config.m_SvTwoEvent && Event == m_ActualEvent[1]) || (m_ActualEvent[0] == ALL && (Event > KATANA || Event < SHOTGUN) && Event != WALLSHOT && Event != SURVIVOR && Event != GRAVITY_0 && Event != GRAVITY_M0_25 && Event != GRAVITY_M0_5 && Event != BULLET_PIERCING && Event != BULLET_GLUE && (Event <= RACE_HUMAN || Event > RACE_RANDOM)))
		{
			return true;
		}
		return false;
