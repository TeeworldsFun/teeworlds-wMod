...
void CGameContext::SetName(int ClientID)
{
	...
	char Prefix[10] = "";
	if (Server()->IsAuthed(ClientID) == AUTHED_ADMIN)
		str_append(Prefix, "A", 10);
	else if (Server()->IsAuthed(ClientID) == AUTHED_MOD)
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if(MsgID == NETMSGTYPE_CL_SAY)
	{
		...
		if(pMsg->m_pMessage[0]=='/')
		{
			...
			{
				char aBuf[256] = "";
				str_format(aBuf, 256, "The race of %s is custom !", m_apPlayers[ClientID]->GetRealName());
				SendChatTarget(-1, aBuf);
