...
void CEvent::Tick()
{
	...
	for ( int i = 0; i < 2; i++ )
	{
		...
		case WEAPON_SLOW:
			...
			break;
		case RACE_HUMAN:
			str_format(aBuf, 256, "%s : All be race Human ! | Remaining : %02d:%02d", Prefix, (150 - Elapsed[i]/Server()->TickSpeed()) / 60, (150 - Elapsed[i]/Server()->TickSpeed()) % 60);
			break;
		case RACE_GNOME:
			str_format(aBuf, 256, "%s : All be race Gnome ! | Remaining : %02d:%02d", Prefix, (150 - Elapsed[i]/Server()->TickSpeed()) / 60, (150 - Elapsed[i]/Server()->TickSpeed()) % 60);
			break;
		case RACE_ORC:
			...
			break;
		case RACE_ELF:
			str_format(aBuf, 256, "%s : All be race Elf ! | Remaining : %02d:%02d", Prefix, (150 - Elapsed[i]/Server()->TickSpeed()) / 60, (150 - Elapsed[i]/Server()->TickSpeed()) % 60);
			break;
		case RACE_RANDOM:
	...
	{
		GameServer()->SendBroadcast(Text, -1, 0);
...
bool CEvent::CanBeUsed(int NewEvent, int Type, bool Canuseactual)
{
	if ( Type == 0 || Type == 1 )
	{
		if (NewEvent == m_ActualEvent[0] && !Canuseactual)
			...
			return false;
		else if (g_Config.m_SvTwoEvent && NewEvent >= RACE_HUMAN && NewEvent <= RACE_RANDOM && m_ActualEvent[1] >= RACE_HUMAN && m_ActualEvent[1] <= RACE_RANDOM)
			return false;
		...
		if ( Type == 1 )
		{
			if ((NewEvent >= GUN && NewEvent <= KATANA) && (m_ActualEvent[0] >= GUN && m_ActualEvent[0] <= KATANA))
				return false;
			else if ((NewEvent >= RACE_HUMAN && NewEvent <= RACE_RANDOM) && (m_ActualEvent[0] >= RACE_HUMAN && m_ActualEvent[0] <= RACE_RANDOM))
				return false;
