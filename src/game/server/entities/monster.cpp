...
void CMonster::Spawn()
{
	...
	GameServer()->m_pController->CanSpawn(0, &SpawnPos);
	/*if(FurthestNum == -1)
    {
	...
	if (m_Ninja.m_CurrentMoveTime > 0)
	{
		...
		// check if we Hit anything along the way
		{
			...
			for (int i = 0; i < Num2; ++i)
			{
				...
				aMonsts[i]->TakeDamage(vec2(0, -10.0f), g_pData->m_Weapons.m_Ninja.m_pBase->m_Damage, m_MonsterID, m_ActiveWeapon, false, true);
			}
		}
	}
}
	...
	switch(m_ActiveWeapon)
	{
		case WEAPON_HAMMER:
		{
			...
			for (int i = 0; i < Num; ++i)
			{
				...
				pTarget->TakeDamage(vec2(0.f, -1.f) + normalize(Dir + vec2(0.f, -1.1f)) * 10.0f, g_pData->m_Weapons.m_Hammer.m_pBase->m_Damage, m_MonsterID, m_ActiveWeapon, false, true);
				Hits++;
			}
            ...
            if(ShotSpread)
            {
                for(int i = -ShotSpread; i <= ShotSpread; ++i)
                {
                    ...
                    CProjectile *pProj = new CProjectile(GameWorld(), WEAPON_GUN,
						...
						1, 0, 0, -1, WEAPON_GUN, false, false, false, 0, true);
		...
		case WEAPON_SHOTGUN:
		{
			...
			for(int i = -ShotSpread; i <= ShotSpread; ++i)
			{
				...
				CProjectile *pProj = new CProjectile(GameWorld(), WEAPON_SHOTGUN,
					...
					1, 0, 0, -1, WEAPON_SHOTGUN, false, false, false, 0, true);
		...
		case WEAPON_GRENADE:
		{
            ...
            if(GrenadeSpread)
            {
                for(int i = -GrenadeSpread; i <= GrenadeSpread; ++i)
                {
					...
					1, true, 0, SOUND_GRENADE_EXPLODE, WEAPON_GRENADE, false, false, false, 0, true);
		...
		case WEAPON_RIFLE:
		{
			...
			if(ShotSpread)
            {
                for(int i = -ShotSpread; i <= ShotSpread; ++i)
                {
                    ...
                    float v = 1-(absolute(i)/(float)ShotSpread);
                    float Speed = mix((float)GameServer()->Tuning()->m_ShotgunSpeeddiff, 1.0f, v);
					new CLaser(GameWorld(), m_Pos, vec2(cosf(a), sinf(a))*Speed, 800.f, m_MonsterID, true);
                }
            }
            else
				new CLaser(GameWorld(), m_Pos, Direction, 800.f, m_MonsterID, true);
...
void CMonster::Die(int Killer)
{
    ...
    if(GameServer()->IsValidPlayer(Killer))
    {
		...
		GameServer()->m_pController->OnCharacterKill(GameServer()->m_apPlayers[Killer], 0);
